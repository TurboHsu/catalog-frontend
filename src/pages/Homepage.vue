<script setup lang="ts">
	...
	const handleReachFooter = async () => {
		...
		if (footerVisibility.value && !isLoadingMore.value) {
...
<template>
	<Dialog>
		<div
			class="flex justify-center items-center h-full w-full min-h-[80vh] overflow-x-hidden"
		>
			<div
				v-if="isLoaded"
			...
			>
				...
				>
					<div v-for="(c, idx) in cats" :key="c.uuid">
						<CatView
							:data="c"
							@click="handleCatViewClick(c)"
							@load="r => (aspectRatios[idx] = r)"
				...
				<div v-if="isLastPage">
					<span class="text-lg font-semibold"
						>No more cats to show</span
					>
				</div>
				<div v-else>
					<Icon
						icon="tdesign:cat"
						class="h-[2rem] w-[2rem] animate-spin"
						ref="footerRef"
					/>
				</div>
			</div>
