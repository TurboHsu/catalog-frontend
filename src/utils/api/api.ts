...
async function addReaction(uuid: string, reaction: string) {
	try {
		...
		const response = await api.get(
			`/api/cat/add_reaction?cat=${uuid}&reaction=${reaction}&fingerprint=${fingerprint}`
		)
...
async function removeReaction(uuid: string, reaction: string) {
	try {
		const fingerprint = await getFingerprint()
		const response = await api.get(
			`/api/cat/remove_reaction?cat=${uuid}&reaction=${reaction}&fingerprint=${fingerprint}`
		)
		const error: string = response.data.error
		if (error && error !== '') {
			throw error
...
async function fetchCats(page: number): Promise<Cat[]> {
	try {
		...
		const response = await fetchWithCache(
			`/api/cat/get?page=${page}&page-size=${import.meta.env.VITE_PAGE_SIZE}&fingerprint=${fingerprint}`
		)
...
async function fetchCat(uuid: string): Promise<Cat> {
	try {
		const fingerprint = await getFingerprint()
		const response = await fetchWithCache(
			`/api/cat/get_by_id?uuid=${uuid}&fingerprint=${fingerprint}`
		)
